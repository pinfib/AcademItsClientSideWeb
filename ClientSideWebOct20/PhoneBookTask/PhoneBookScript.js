$(document).ready(function () {
    ...
    $("#add_contact_button").click(function () {                // добавление новой записи
        ...
        checkInput(firstName, /[^a-zа-я]/i, firstName.val());
        checkInput(lastName, /[^a-zа-я]/i, lastName.val());
        checkInput(phoneNumber, /[^0-9#\*]/, phoneNumber.val());
    ...
    function addElement(firstName, lastName, phoneNumber) {
        ...
        tableRow.append($("<td>").html("<button title='Удалить текущий контакт' class='delete_row_button'>x</button>"));
    ...
    function checkInput(pageElement, regularExpression, value) {
        if (value === "" || value === null) {
    ...
    $("#delete_marked_button").click(function () {
        var deletedElementsCount = $('input[type=checkbox]:checked').length;

        if (deletedElementsCount > 0 && confirm("Вы уверены, что хотите удалить отмеченные записи?")) {
